...
def spawn_workers(main, cfg):
    ...
    if isinstance(cfg.CONFIG.VISIBLE_GPU, int):
        visible_gpu = f'{cfg.CONFIG.VISIBLE_GPU}'
    else:
        for gpu in cfg.CONFIG.VISIBLE_GPU:
            visible_gpu += f'{gpu},'
...
def main_worker(gpu, ngpus_per_node, main, cfg):
    ...
    if cfg.DDP_CONFIG.DISTRIBUTED:
        ...
        dist.init_process_group(backend=cfg.DDP_CONFIG.DIST_BACKEND, init_method=cfg.DDP_CONFIG.DIST_URL, world_size=cfg.DDP_CONFIG.GPU_WORLD_SIZE, rank=cfg.DDP_CONFIG.GPU_WORLD_RANK)
    main(cfg)
